...
class HeapBlock {
    ...
    emulator;
    rom;
    constructor(rom, emulator, pointer) {
        this.emulator = emulator;
...
class RomHeap {
    ...
    emulator;
    rom;
    constructor(rom, emulator, start = 0, size = 0) {
        let block;
        ...
        while (block.pointer) {
            ...
            if (block.free >= size) {
                ...
                if (new_block.free) {
                    ...
                    if (block.free_next)
                        new HeapBlock(this.rom, this.emulator, new_block.free_next).free_last = new_block.pointer;
                ...
                block.free = 0;
                if (block.free_last)
                    new HeapBlock(this.rom, this.emulator, block.free_last).free_next = block.free_next;
                if (block.free_next)
                    new HeapBlock(this.rom, this.emulator, block.free_next).free_last = block.free_last;
                ...
                if (block.used_next)
                    new HeapBlock(this.rom, this.emulator, new_block.used_next).used_last = new_block.pointer;
    ...
    free(address) {
        ...
        if (!used_last.free) {
            ...
            if (block.free_last)
                free_last = new HeapBlock(this.rom, this.emulator, block.free_last);
            else {
            ...
            if (free_last) {
                ...
                if (free_last.free_next)
                    new HeapBlock(this.rom, this.emulator, used_last.free_next).free_last = used_last.pointer;
                ...
                if (this.free_head)
                    new HeapBlock(this.rom, this.emulator, this.free_head).free_last = used_last.pointer;
                else
                    this.free_tail = used_last.pointer;
        ...
        if (block.used_next)
            new HeapBlock(this.rom, this.emulator, block.used_next).used_last = block.used_last;
...
exports.RomHeap = RomHeap;
//# sourceMappingURL=romheap.js.map